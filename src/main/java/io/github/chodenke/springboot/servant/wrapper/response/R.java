 ...
 * @since JDK 17
 */
public final class R<T> extends SucceededResponseWrapper<T> {
    ...
    public static <T> R<T> ok(T data, String code) {
        return new R<>(data, code, DEFAULT_SUCCESS_MSG);
    }
     ...
     */
    public static <T> R<T> ok(T data, ISuccessEnumBase successEnum) {
        return new R<>(data, successEnum.getIntegratedCode(), successEnum.getDesc());
    }
